...
import { buildAccumulatedSeries, computeGeneralBenchmark } from '@/utils/accumulatedInflation'
...
function getDefaultSelection(series: RubricaSeries[], baseMonth: string, endMonth: string): string[] {
  const ranked = series
    .map(item => ({
      id: item.id,
    ...
    .sort((a, b) => Math.abs((b.endAccumulated ?? 0)) - Math.abs((a.endAccumulated ?? 0)))
...
export default function RubricasExplorer() {
  ...
  const [isWideViewport, setIsWideViewport] = useState(() => window.innerWidth >= 1024)
  ...
  const timeline = useMemo(
    () => rubricasData ? rubricasData.months.filter(month => month >= baseMonth) : [],
    [baseMonth, rubricasData]
  )
  const defaultEndMonth = timeline[timeline.length - 1] ?? ''

  const classSeries = useMemo(
    () => rubricasData ? rubricasData.series.filter(item => item.level === 'clase') : [],
    [rubricasData]
  )
  const generalSeries = useMemo(
    () => rubricasData?.series.find(item => item.level === 'grupo' && item.codigo === '00'),
    [rubricasData]
  )

  const subgroupCatalog = useMemo(
    () => new Map((rubricasData?.catalog ?? []).filter(node => node.level === 'subgrupo').map(node => [node.id, node])),
    [rubricasData]
  )
  const groupCatalog = useMemo(
    () => new Map((rubricasData?.catalog ?? []).filter(node => node.level === 'grupo').map(node => [node.id, node])),
    [rubricasData]
  )
  ...
  useEffect(() => {
    if (!rubricasData || timeline.length === 0 || initializedRef.current) return

    const urlState = parseRubricasURLState()
    const validIds = new Set(classSeries.map(item => item.id))

    const nextFrom = urlState.fromMonth && timeline.includes(urlState.fromMonth)
      ? urlState.fromMonth
      : baseMonth
    const rawTo = urlState.toMonth && timeline.includes(urlState.toMonth)
      ? urlState.toMonth
      : defaultEndMonth
  ...
  }, [baseMonth, defaultEndMonth, fromMonth, rubricasData, selectedSeriesIds, toMonth])
  ...
  const filteredSeries = useMemo(() => {
    ...
    const rows = classSeries.map(item => {
      const subgroup = item.parentRubricaId ? subgroupCatalog.get(item.parentRubricaId) : undefined
      const group = subgroup?.parentIds?.[0] ? groupCatalog.get(subgroup.parentIds[0]) : undefined
    ...
    return rows
      .filter(row => {
        const target = `${row.series.codigo} ${row.series.nombre} ${row.subgroupName} ${row.groupName}`.toLowerCase()
  ...
  const selectedSeries = useMemo(
    () => selectedSeriesIds.map(id => classSeriesById.get(id)).filter((item): item is RubricaSeries => Boolean(item)),
  ...
  const seriesLines = useMemo(() => {
    return selectedSeries.map(item => ({
      ...
      accumulated: buildAccumulatedSeries(item.points, baseMonth, visibleMonths),
    }))
  }, [baseMonth, selectedSeries, visibleMonths])

  const benchmarkValue = useMemo(() => {
    if (!generalSeries || visibleMonths.length === 0) return null
    return computeGeneralBenchmark(generalSeries.points, baseMonth, visibleMonths[visibleMonths.length - 1])
  ...
  const latestSeriesValues = useMemo(() => {
    ...
    return seriesLines
      .map(line => {
        const lastPoint = line.accumulated.find(item => item.month === lastVisibleMonth)
        return {
          ...line,
  ...
  const limitMessage = !canAddMore ? `Límite alcanzado: máximo ${MAX_SELECTED_SERIES} rúbricas.` : ''
  ...
  const handleToggleSeries = (seriesId: string) => {
    setSelectedSeriesIds(prev => {
      ...
      if (prev.length >= MAX_SELECTED_SERIES) {
        setSelectionNotice(`Límite alcanzado: máximo ${MAX_SELECTED_SERIES} rúbricas.`)
        return prev
      }
      setSelectionNotice('')
  ...
  const resetSelection = () => {
    setSelectionNotice('')
    setSelectedSeriesIds(getDefaultSelection(classSeries, baseMonth, toMonth || defaultEndMonth))
  }
      ...
      <Card className="border-primary/30 bg-primary/5">
        <CardContent className="py-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Selecciona hasta {MAX_SELECTED_SERIES} clases ECOICOP y compáralas contra la referencia del IPC general.
          </p>
        </CardContent>
      </Card>
        ...
        <Card className="min-w-0">
          <CardHeader className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground" id={selectionStatusId} aria-live="polite">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                <label htmlFor={fromMonthId} className="text-xs font-medium text-muted-foreground">
                  Desde
                </label>
              ...
              <div className="space-y-1">
                <label htmlFor={toMonthId} className="text-xs font-medium text-muted-foreground">
                  Hasta
                </label>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="ghost" size="sm" onClick={clearSelection} disabled={selectedSeriesIds.length === 0}>
                Limpiar
              </Button>
            </div>
          </CardHeader>
          <CardContent>
            <div id={listId} className="max-h-[28rem] space-y-2 overflow-auto pr-1" role="group" aria-label="Listado de rúbricas disponibles">
        ...
        <Card className="min-w-0">
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Benchmark horizontal (rojo): IPC general acumulado hasta {formatMonth(toMonth)}
          ...
          <CardContent className="space-y-4">
            {selectedSeries.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={420}>
                <LineChart data={chartData} margin={{ top: 8, right: isWideViewport ? 190 : 16, left: 0, bottom: 12 }}>
                    ...
                    formatter={(value: number | string | Array<number | string>, name: string | number) => {
                      const numericValue = typeof value === 'number'
                        ...
                        : Array.isArray(value)
                          ? null
                          : Number(value)
                      const line = seriesLines.find(item => item.dataKey === String(name))
                      return [
                        numericValue == null || Number.isNaN(numericValue) ? 'Sin dato' : `${numericValue.toFixed(1)}%`,
                  ...
                  {latestSeriesValues.map(item => (
                    ...
                    >
                      <span
                        ...
                        aria-hidden="true"
                      />
                      <span className="max-w-[16rem] truncate">{item.code} · {item.name}</span>
            ...
            <p className="text-xs text-muted-foreground">
              Fuente: INE (IPC, índices mensuales ECOICOP). Base fija para acumulado: {formatMonth(baseMonth)}.
            </p>
          </CardContent>
        </Card>
      </div>
