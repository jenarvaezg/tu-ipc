...
export default function TabNavigation({
        ...
        return (
          <Button
            ...
            className={cn(
              'relative',
              mobile ? 'w-full px-2' : '',
              !isActive ? 'hover:bg-primary/10 hover:text-primary transition-colors' : ''
            )}
          >
            <span className="truncate">{tab.label}</span>
            {showDesgloseBadge && tab.id === 'desglose' && (
              <>
                <span className="ml-1 hidden md:inline-flex rounded-full bg-primary/15 px-1.5 py-0.5 text-[10px] font-semibold text-primary">
                  Personalizar
                </span>
                <span className="ml-1 inline-flex h-2 w-2 rounded-full bg-primary md:hidden" aria-hidden="true" />
  ...
  if (mobileSticky) {
    return (
      <>
        <div className="mb-4 hidden lg:block" role="tablist" aria-label="Secciones de la calculadora">
          {renderTabs(false)}
        </div>
