...
export default function SalaryCalculator({ personalIPC, startMonth, endMonth }: SalaryCalculatorProps) {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Compara tu evolución salarial con tu inflación personal ({startMonth} a {endMonth})
          </CardDescription>
        </CardHeader>
            ...
            <div className="flex gap-2">
              {PAY_TYPES.map(pt => (
                <Button
                  ...
                  size="sm"
                  onClick={() => setInputs(prev => ({ ...prev, payType: pt.id }))}
                  className={
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="animate-slide-up" style={{ animationDelay: '0.05s' }}>
            <CardContent className="pt-6 text-center">
              <p className="text-xs font-medium text-muted-foreground mb-1">Subida salarial</p>
              <p className={`text-2xl font-bold ${result.nominalGrowth >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                {result.nominalGrowth >= 0 ? '+' : ''}{result.nominalGrowth.toFixed(2)}%
              </p>
            </CardContent>
          </Card>
          <Card className="animate-slide-up" style={{ animationDelay: '0.1s' }}>
            <CardContent className="pt-6 text-center">
              <p className="text-xs font-medium text-muted-foreground mb-1">Tu inflación</p>
              <p className={`text-2xl font-bold ${personalIPC >= 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
          ...
          <Card className="animate-slide-up" style={{ animationDelay: '0.15s' }}>
            <CardContent className="pt-6 text-center">
              <p className="text-xs font-medium text-muted-foreground mb-1">Cambio real</p>
              <p className={`text-2xl font-bold ${result.realGrowth >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                {result.realGrowth >= 0 ? '+' : ''}{result.realGrowth.toFixed(2)}%
              </p>
              <p className={`text-xs mt-1 ${result.realGrowth >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                {result.realGrowth >= 0 ? 'Tu sueldo le gana a la inflación' : 'La inflación le gana a tu sueldo'}
              </p>
            </CardContent>
          </Card>
          <Card className="animate-slide-up" style={{ animationDelay: '0.2s' }}>
            <CardContent className="pt-6 text-center">
              <p className="text-xs font-medium text-muted-foreground mb-1">Poder adquisitivo</p>
              <p className={`text-2xl font-bold ${result.monthlyChange >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                {result.monthlyChange >= 0 ? '+' : ''}{result.monthlyChange}€/mes
              </p>
            </CardContent>
          </Card>
        </div>
